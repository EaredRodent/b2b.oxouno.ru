const wsHelper = new function () {
  this.findIntersectionOfSubscriptionsWithModifiedTables = function (store) {
    ...
    let modifiedTables = store.state.modifiedTables
    let wsSubscriptions = store.state.wsSubscriptions

    for (let wsSubscription of wsSubscriptions) {
      // Поиск пересечения с измененными таблицами
      intersection = wsSubscription.tables.some((table) => {
        return modifiedTables.indexOf(table) !== -1
      })
  ...
  this.executeSubscription = function (store, wsSubscription) {
    for (let callback of wsSubscription.callbacks) {
      callback()
    }
  }
}()
...
export const mutations = {
  addWsSubscription (state, { id, tables = [], callbacks = [] }) {
  ...
  removeWsSubscription (state, id) {
    let idx = state.wsSubscriptions.findIndex(el => el.id === id)
...
export const actions = {
  ...
  registerWsSubscription (store, { tables = [], callbacks = [] }) {
    return new Promise((resolve, reject) => {
      let id = store.state.lastSubscriptionId
